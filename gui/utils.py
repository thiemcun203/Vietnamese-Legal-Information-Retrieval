            ...
            if law['law_id'] == law_id:
                for article in law['articles']:
                    if article['article_id'] == article_id:
                        documents['full_content'] += f'''{law_id} - {article_id}
{article['title']}
{article['text']}\n\n'''    

    annotations = []
    doc_ids = [doc_id for id in retrived_id for doc_id in retrived_id[id] ]
    for rank, doc_id in enumerate(doc_ids):
        tag = calculate_highlight_tag(rank, len(doc_ids))
        start_index, end_index = find_index(documents['full_content'].lower(), doc_id.lower())
        if start_index != -1:
