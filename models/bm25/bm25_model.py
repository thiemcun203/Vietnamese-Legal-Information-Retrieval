...
class BM25:
    ...
    def search(self, query:str, preprocess_query=True):
        ...
        """
        if preprocess_query:
            query = bm25_preprocess(query, skip_word_segment=True)
    ...
    def query(self, segmented_question:str, preprocess_query=True, limit:int=10):
        scores = self.search(segmented_question, preprocess_query=preprocess_query)
        top_ids = sorted(range(len(scores)), key=lambda i: scores[i], reverse=True)[:limit]
        content_results = {}
        for idx in top_ids:
            doc = self.split_lookup[idx]
            key  = doc['law_article_id']
            if key not in content_results:
                content_results[key] = [idx]
            else:  
                content_results[key].append(idx)
